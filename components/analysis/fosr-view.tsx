...
import { runFOSR } from "@/lib/techniques/supervised/fosr";
...
export default function FOSRView({ functionalDataset }: { functionalDataset?: FunctionalDataset }) {
  const [responseData, setResponseData] = useState<number[] | null>(null);
  const [nComponents, setNComponents] = useState(5);
  const [results, setResults] = useState<any | null>(null);
  const [isRunning, setIsRunning] = useState(false);
  
  // Selection state for box select functionality
  ...
  const handleRun = async () => {
    ...
    setIsRunning(true);
    try {
      const res = await runFOSR(functionalDataset.data, responseData, {
        nComponents,
        center: true,
        scaleScores: false,
      });
      ...
      console.error("Error running FOSR:", error);
    } finally {
      setIsRunning(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Function-on-Scalar Regression Configuration</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center gap-4">
            ...
            <Button 
              ...
              className="mt-6"
            >
              {isRunning ? "Running Analysis..." : "Run FOSR Analysis"}
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      {results && (
        <Card>
          <CardHeader>
            <CardTitle>FOSR Results</CardTitle>
          </CardHeader>
          <CardContent>
            <Tabs defaultValue="summary" className="w-full">
              ...
              <TabsContent value="residuals" className="space-y-4">
                ...
                {/* Functional Curves with selection highlighting */}
                <div>
