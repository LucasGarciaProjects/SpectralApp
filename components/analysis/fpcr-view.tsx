...
/**
 * Vista de Regresión de Componentes Principales Funcional (FPCR)
 * 
 * Este componente implementa regresión lineal con datos funcionales usando
 * componentes principales funcionales. Incluye análisis de residuos, parámetro
 * funcional y visualizaciones de curvas ajustadas.
 */
...
import { runFPCR } from "@/lib/techniques/supervised/fpcr";
...
export default function FPCRView({ functionalDataset }: { functionalDataset?: FunctionalDataset }) {
  ...
  useEffect(() => {
    if (rawSelectionData && selectionLocked && plotRef.current) {
      // Small delay to ensure plot is ready
      setTimeout(() => {
  ...
  }, [rawSelectionData, selectionLocked])
  ...
  const handleRun = async () => {
    ...
    setIsRunning(true);
    try {
      const res = await runFPCR(functionalDataset.data, responseData, {
        nComponents,
        center: true,
        scaleScores: false,
      });
      ...
      console.error("Error running FPCR:", error);
    } finally {
      setIsRunning(false);
    }
  };
  ...
  const downloadFittedValues = () => {
    ...
    a.href = url;
    a.download = "fpcr_fitted_values.csv";
    a.click();
  ...
  const downloadBetaFunction = () => {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = "fpcr_beta_function.csv";
    a.click();
    URL.revokeObjectURL(url);
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center gap-4">
            ...
            <Button 
              ...
              className="mt-6"
            >
              {isRunning ? "Running Analysis..." : "Run Regression Analysis"}
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      {results && (
        <Card>
          <CardHeader>
            <CardTitle>FPCR Results</CardTitle>
          </CardHeader>
          <CardContent>
                    ...
                    <div className="h-80">
                      <Plot
                        ...
                        layout={{
                          ...
                          xaxis: { title: "Observed Values" },
                          yaxis: { title: "Fitted Values" },
                        ...
                        onSelected={(event: any) => {
                          ...
                          try {
                            console.log('FPCR onSelected event:', event) // Debug
                            const points = event?.points || []
                            const indices = points.map((p: any) => p.pointIndex ?? p.pointNumber).filter((i: any) => typeof i === 'number')
                            console.log('FPCR selected indices:', indices) // Debug
                            ...
                            console.warn('FPCR Selection error:', error)
                          }
                        }}
                        onDeselect={() => {
                    ...
                    <div className="h-80">
                      <Plot
                        data={[
                          ...
                          {
                            x: results.predictions,
                            y: new Array(results.predictions.length).fill(0),
                            type: "scatter",
                            mode: "lines",
                            name: "Zero Line",
                            line: { color: "red", dash: "dash" }
